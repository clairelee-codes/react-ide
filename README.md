...
- **Youtube(2년 전) 코드가 됐던 이유**: 강의 버전은 `[...folders]`로 얕은 복사만 한 뒤 `newFolders[i].files[j].code = ...`처럼 **state를 직접 mutate**했음. 얕은 복사라 내부 객체 참조가 동일해서, mutate하는 순간 원본 `folders` 객체도 같이 바뀌어버려 우연히 타이밍 문제를 회피한 것 (권장되는 방식은 아님 — 참조 동일성에 의존하는 다른 최적화들과 충돌 위험).
...
- 근본적으로 반복되는 문제라면 **state 중복 저장을 줄이고 연관된 전환을 원자적으로 묶는 것(`useReducer` 등)** 이 구조적 해법.
